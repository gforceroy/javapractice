def subsetTarget(arr, indx, sum_1, sum_2, target_1, target_2, hasTarget):
    if(indx >= len(arr)):
        return hasTarget and sum_1 == target_1 and sum_2 == target_2

    return subsetTarget(arr, indx + 1, sum_1 + arr[indx], sum_2, target_1, arr[indx], True) \
        or subsetTarget(arr, indx + 1, sum_1 + arr[indx], sum_2, target_1, target_2, hasTarget) \
        or subsetTarget(arr, indx + 1, sum_1 , sum_2 + arr[indx], arr[indx] , target_2,
         True) \
        or subsetTarget(arr, indx + 1, sum_1 , sum_2 + arr[indx], target_1, target_2, hasTarget)
...
array_8 = [35, 53, 62, 18, -44, -55, 19]
print("YES" if subsetTarget(array_8,0, 0, 0, 0, 0, False) else "NO")
